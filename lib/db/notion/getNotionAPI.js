...
const useRateLimiter = process.env.BUILD_MODE === 'true' || isExport()
...
function getRawNotion() {
  if (!globalStore.notion) {
    const tokenV2Configured = Boolean(BLOG.NOTION_TOKEN_V2)
    const activeUserConfigured = Boolean(BLOG.NOTION_ACTIVE_USER)

    console.info(
      '[NotionAuth]',
      JSON.stringify({
        mode:
          tokenV2Configured && activeUserConfigured
            ? 'authenticated-session'
            : 'public-page',
        tokenV2Configured,
        activeUserConfigured,
        complete: tokenV2Configured === activeUserConfigured
      })
    )
