...
const READ_CACHE_MAX_AGE = 30 * 60 * 1000
...
const getCurrentUrl = url => {
  ...
  try {
    ...
    // Public reading counts belong to the canonical page. Tracking parameters
    // and fragments must not split one article into multiple counters.
    parsed.search = ''
    parsed.hash = ''
...
const getFreshCachedCounts = (url, maxAge = READ_CACHE_MAX_AGE) => {
  const cached = readCache(url)
  const page = cached?.pages?.[getPageKey(url)]
  const updatedAt = Number(page?.updated_at)

  if (
    !Number.isFinite(updatedAt) ||
    Date.now() - updatedAt > Math.max(0, maxAge)
  ) {
    return null
  }

  const counts = getCachedCounts(url)
  return normalizeCount(counts?.page_pv) === null ? null : counts
}
...
const fetch = async ({ url, requestTimeout = REQUEST_TIMEOUT } = {}) => {
  ...
  try {
    ...
    // Only remember this browser after Vercount accepted the first visit.
    // Otherwise a transient failure would suppress its UV for a full year.
    if (isNewUv) {
      setUvCookie()
    }
...
/**
 * Read a page's counters without recording a visit.
 * Vercount's GET endpoint is read-only; POST is reserved for normal page views.
 */
const read = async ({
  url,
  requestTimeout = REQUEST_TIMEOUT,
  maxAge = READ_CACHE_MAX_AGE
} = {}) => {
  if (typeof window === 'undefined' || typeof window.fetch !== 'function') {
    return null
  }
  ...
  const cached = getFreshCachedCounts(currentUrl, maxAge)
  if (cached) return cached
  ...
  try {
    const response = await window.fetch(
      `${API_URL}?url=${encodeURIComponent(currentUrl)}`,
      {
        method: 'GET',
        signal: controller.signal
      }
    ...
    if (!response.ok) {
      throw new Error(`Vercount read failed with HTTP ${response.status}`)
    }
    ...
    const cachedCounts = getCachedCounts(currentUrl)
    return normalizeCount(cachedCounts?.page_pv) === null ? null : cachedCounts
  } finally {
    window.clearTimeout(timeoutId)
  }
}
