...
const isVercelRuntime = !isBuildPhase && Boolean(process.env.VERCEL_ENV)
const enableLocalCache = isBuildPhase || !isVercelRuntime
...
function isUsableCacheValue(key, data) {
  return isCacheableCacheValue(key, data)
}

async function recoverLastKnownGoodAfterSourceFailure(key, sourceError) {
  if (!isVercelRuntime || !isCriticalNotionCacheKey(key)) {
    throw sourceError
  }

  try {
    const fallback = await VercelCache.getLastKnownGoodCache(key)
    if (isUsableCacheValue(key, fallback)) {
      cacheLog(
        'SOURCE-FALLBACK',
        key,
        `source refresh failed: ${sourceError.message}`
      )
      return fallback
    }
  } catch (fallbackError) {
    console.warn(
      `[Cache] last-known-good read failed key:${key}`,
...
export async function getOrSetDataWithCustomCache(
  ...
  if (isBuildPhase) {
    const promise = withFileLock(
      ...
      async () => {
        ...
        const data = assertCacheableSourceValue(
          key,
          await getDataFunction(...getDataArgs)
        )
        if (isUsableCacheValue(key, data)) {
  ...
  const promise = (async () => {
    try {
      const sourceData = await getDataFunction(...getDataArgs)
      const data = assertCacheableSourceValue(key, sourceData)
      if (isUsableCacheValue(key, data)) {
        await setDataToCache(key, data, customCacheTime)
      ...
      cacheLog('ERROR', key, err.message)
      return recoverLastKnownGoodAfterSourceFailure(key, err)
    } finally {
...
export async function setDataToCache(key, data, customCacheTime) {
  if (!isUsableCacheValue(key, data)) {
    if (isCriticalNotionCacheKey(key)) {
      console.warn(`[Cache] skipped invalid Notion snapshot key:${key}`)
    }
    return
  }
  ...
  for (const { name, api } of chain) {
    try {
      ...
      if (isCriticalNotionCacheKey(key) && data != null) {
        cacheLog('INVALID', key, `${name} entry rejected and deleted`)
