...
const normalizeLedgerEvent = event => {
  ...
  return {
    id: event?.id || event?.eventId || `${type}:${repositoryId}:${timestampMs}`,
    type,
    repositoryId,
 ...
 *
 * Historical updates only come from an append-only event source. Notion's
...
export function buildContributionEvents({ posts = [], ledgerEvents = [] } = {}) {
  ...
  candidates.forEach(event => {
    const semanticKey = `${event.type}:${event.repositoryId}:${event.timestampMs}`
    if (!deduplicated.has(semanticKey)) {
      deduplicated.set(semanticKey, event)
    }
  })
